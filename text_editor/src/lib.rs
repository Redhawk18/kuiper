//! Based off iced's text editor with required changes.
use iced::{
    ...
    advanced::{
        ...
        graphics::core::SmolStr,
        graphics::core::{InputMethod, input_method, widget},
        layout, mouse, renderer,
        text::{
            self, Editor as _, LineHeight, Text, Wrapping,
            editor::Cursor,
    ...
    Renderer: text::Renderer,
{
    ...
    fn input_method<'b>(
        ...
        let cursor = match internal.editor.cursor() {
            ...
            Cursor::Selection(ranges) => ranges.first().cloned().unwrap_or_default().position(),
        };
        ...
        let position = cursor + translation + Vector::new(0.0, f32::from(line_height));
