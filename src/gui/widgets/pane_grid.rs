...
pub fn pane_grid<'a>(state: &'a State<PaneState>, active: usize, data: &'a [Tab]) -> PaneGrid<'a, Message, Renderer> {
    PaneGrid::new(state, |pane, state, is_maximized| {
        Content::new(match state {
            PaneState::Tab => tab_bar(active, data),
        })
        .title_bar(TitleBar::new("content").style(crate::gui::theme::Container::PaneGridTitleBar(true)))
    })
